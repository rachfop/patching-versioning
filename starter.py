...
async def main():
    ...
    if args.start_workflow:
        handle = await client.start_workflow(
            BackgroundCheck.run, id=args.start_workflow, task_queue="backgroundcheck-boilerplate-task-queue-local"
        )
        print(f"Started workflow with ID {handle.id} and run ID {handle.result_run_id}")
    if args.query_workflow:
        handle = client.get_workflow_handle_for(BackgroundCheck.run, args.query_workflow)
